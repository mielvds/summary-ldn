  ...
  constructor(file) {
    ...
    hdt.fromFile(file).then(async doc => {
      ...
      for (const term of terms) {
        //console.log(`-- Processing summary for ${term}`);
        ...
        while (true) {
          ...
          for (const triple of triples) {
            ...
            if (Util.isNamedNode(RdfString.stringToTerm(triple.object))) {
              if (term === rdf + "type") {
        ...
        this.push(
          quad(capability, namedNode(ds + "predicate"), namedNode(term))
        );

        // add authorities
        subjects.forEach(s =>
          this.push(
            quad(capability, namedNode(ds + "sbjAuthority"), namedNode(s))
          )
        );

        objects.forEach(o =>
          this.push(
            quad(capability, namedNode(ds + "objAuthority"), namedNode(o))
          )
        );
