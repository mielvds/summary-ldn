...
class SummaryStream extends Readable {
        ...
        while (true) {
          ...
          for (const triple of triples) {
            ...
            if (Util.isNamedNode(triple.subject)) {
              const authority = this._getDomain(triple.subject.value);
              subjects.add(authority);
            }
              

            if (Util.isNamedNode(triple.object)) {
              if (term.equals(namedNode(rdf + "type"))) {
                objects.add(triple.object);
              } else {
                const authority = this._getDomain(triple.object.value);
  ...
  _constructFilter(objects) {
    ...
    uris.forEach(uri => filter.add(Buffer.from(uri.id)));
